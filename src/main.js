  ...
  <main id="top">
    <section class="hero site-shell">
      <div class="hero-copy">
        ...
        <p class="hero-lead">
          想象全国每个地点原本都要存经纬度。超压缩做的事，是先铺一条可以覆盖空间的“数学高铁线”，
    ...
    <section id="quantization" class="section site-shell">
      ...
      <div class="qa-band">
        <div>
          <h3>老师可能会问：HC 文中提到 INR，是不是用 INR 做 HF 产生权重？</h3>
          <p>
            可以回答：它借用了 INR 的“用函数隐式表示数据”的思想，但核心实现不是常规 INR 训练一个神经网络来生成权重。
            论文里的 hyperfunction 主要通过无理数缠绕构造高维空间中的密集轨迹，再把权重块投到这条轨迹附近。
          </p>
        </div>
        <div>
          <h3>那 HC 中的 HF 不是 INR，它是什么？</h3>
          <p>
